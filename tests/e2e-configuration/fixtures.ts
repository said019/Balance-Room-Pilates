...
export const origin='http://127.0.0.1:3531';
const u=new URL(process.env.DATABASE_URL||'');
if(u.hostname!=='127.0.0.1'||u.port!=='54360'||u.pathname!=='/altitud_2707'||u.username!=='altitud_2707_app')throw new Error('Disposable configuration database only');
...
export const test=base.extend<{fixture:Fixture}>({
 ...
 const oldOperational=(await pool.query('SELECT * FROM operational_settings WHERE id=true')).rows[0];
 const oldFounding=(await pool.query('SELECT * FROM founding_policy WHERE singleton=true')).rows[0];
 ...
 try {
  ...
  await pool.query('DELETE FROM operational_settings_audit WHERE actor_id=$1',[ids.admin]);
  await pool.query('UPDATE operational_settings SET version=$1,cancellation_hours=$2,booking_advance_days=$3,max_active_bookings=$4,max_bookings_per_day=$5,pending_notes=$6,updated_by=$7,updated_at=$8 WHERE id=true',[oldOperational.version,oldOperational.cancellation_hours,oldOperational.booking_advance_days,oldOperational.max_active_bookings,oldOperational.max_bookings_per_day,JSON.stringify(oldOperational.pending_notes),oldOperational.updated_by,oldOperational.updated_at]);
  await pool.query('UPDATE founding_policy SET version=$1,mode=$2,updated_by=$3,updated_at=$4 WHERE singleton=true',[oldFounding.version,oldFounding.mode,oldFounding.updated_by,oldFounding.updated_at]);
  ...
  await pool.query('DELETE FROM order_health_acceptances WHERE user_id=ANY($1::uuid[])',[[ids.client,ids.other]]);
