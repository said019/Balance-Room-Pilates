...
it('requires selecting a valid target instead of cancelling first',async()=>{(api.get as jest.Mock).mockImplementation(async(path:string)=>({data:path==='/operational-settings/public'?{cancellation_hours:6,version:1}:[target]}));mount();await screen.findByText(/TRAIN destino/);expect(screen.getByRole('button',{name:'Confirmar cambio'})).toBeDisabled();});
it('keeps the original reservation dialog recoverable after an atomic change is rejected',async()=>{(api.get as jest.Mock).mockImplementation(async(path:string)=>({data:path==='/operational-settings/public'?{cancellation_hours:6,version:1}:[target]}));const change=jest.fn().mockRejectedValue(new Error('El destino se llenó'));const {onClose}=mount(change);await userEvent.selectOptions(await screen.findByLabelText('Nueva sesión'),'target-class');await userEvent.click(screen.getByRole('button',{name:'Confirmar cambio'}));await screen.findByText('El destino se llenó');expect(onClose).not.toHaveBeenCalled();expect(change).toHaveBeenCalledWith(booking,target);});
