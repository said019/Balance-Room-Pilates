...
import { authUrl, safeReturnUrl } from '@/lib/auth-redirect';
...
export default function AuthPage({ mode }: { mode: Mode }) {
  ...
  return <main className="alt-site alt-auth"><section className="alt-auth-image"><img className="alt-auth-photo" src="/brand/hybrid-training.jpg" alt="Entrenamiento híbrido en comunidad" /><Link to="/" className="alt-auth-logo" aria-label="Volver a 2707 Altitud"><img src="/brand/logo-light.svg" alt="2707 Altitud" width="280" height="140" /></Link><div className="alt-auth-manifesto"><div className="alt-eyebrow">2707 M S. N. M. · ZINACANTEPEC</div><h1>EL SIGUIENTE<br />NIVEL ES<br /><span>CONTIGO.</span></h1><p>Disciplina. Comunidad. Evolución.</p></div></section><section className="alt-auth-content"><Link to="/" className="alt-auth-back">← Volver al studio</Link><div className="alt-auth-form"><div className="alt-eyebrow">PERFORMANCE MEETS LIFESTYLE</div><h2>{title}</h2><p>{mode==='login'?(bookingIntent?'Inicia sesión para reservar tu clase. Al entrar, te llevaremos a la agenda.':'Entra a tu cuenta y haz espacio para entrenar.'):mode==='register'?'Únete a una comunidad que te impulsa a ir más lejos.':'Escribe tu correo para recuperar tu contraseña.'}</p>{!configured&&<div className="alt-auth-notice">Estamos preparando el acceso al nuevo studio. <Link to="/app/preview">Explora tu app de usuario ↗</Link></div>}{success?<div className="alt-auth-success" role="status"><h3>Revisa tu correo.</h3><p>Si existe una cuenta con ese correo, recibirás las instrucciones para restablecer tu contraseña.</p><Link to={authUrl('/login', returnUrl)} className="alt-text-link">Volver al inicio de sesión</Link></div>:<form onSubmit={submit}>
    ...
    {mode!=='forgot'&&<div className="alt-form-field"><div className="alt-field-label"><label htmlFor="password">Contraseña</label>{mode==='login'&&<Link to={authUrl('/forgot-password', returnUrl)}>¿La olvidaste?</Link>}</div><div className="alt-password-input"><input id="password" name="password" type={visible?'text':'password'} autoComplete={mode==='login'?'current-password':'new-password'} minLength={mode==='register'?8:1} pattern={mode==='register'?'(?=.*[A-Z])(?=.*[0-9]).{8,}':undefined} required disabled={busy} aria-describedby={mode==='register'?'password-help':undefined} /><button type="button" onClick={()=>setVisible(!visible)} aria-label={visible?'Ocultar contraseña':'Mostrar contraseña'}>{visible?'Ocultar':'Mostrar'}</button></div>{mode==='register'&&<small id="password-help">Mínimo 8 caracteres, una mayúscula y un número.</small>}</div>}
    ...
    <div className="alt-auth-switch">{mode==='login'?<>¿Aún no eres parte? <Link to={authUrl('/register', returnUrl)}>Crea tu cuenta ↗</Link></>:<Link to={authUrl('/login', returnUrl)}>Ya tengo cuenta. Iniciar sesión ↗</Link>}</div><div className="alt-auth-signoff">UN LUGAR. UN NIVEL. TU SIGUIENTE VERSIÓN.</div></div></section></main>;
}
